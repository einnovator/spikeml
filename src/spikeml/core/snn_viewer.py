...
class SSNNMonitorViewer(SensingMonitorViewer):
    ...
    def render(self, options=None):
        _,axs = self._axes(3) 
        tt = self._signal_changes()
        self._plot_spikes('zy', tt=tt, callback=lambda ax: self._plot_input(ax), options=options, ax=axs[0])
        self._plot_xt(['y'], options=options, ax=axs[1])
...
class LIConnectorMonitorViewer(ConnectorMonitorViewer):
    ...
    def render(self, options: Optional[Union[dict, List[str], str]] = None) -> None:
        ...
        tt = self._signal_changes()
        self._plot_spikes(['Wp'], tt=tt, callback=lambda ax: self._plot_input(ax), options=options, ax=axs[0])
        self._plot_spikes(['Wn'], tt=tt, callback=lambda ax: self._plot_input(ax), options=options, ax=axs[1])
        self._plot_spikes(['Zp'], tt=tt, callback=lambda ax: self._plot_input(ax), options=options, ax=axs[2])
        self._plot_spikes(['Zn'], tt=tt, callback=lambda ax: self._plot_input(ax), options=options, ax=axs[3])
